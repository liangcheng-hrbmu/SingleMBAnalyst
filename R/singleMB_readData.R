...
read_Host <- function(named.outs.dirs) {
...
#' @param align.method Default: \code{"kraken2"}. SingleMB alignment methods, selected from ["kraken2", "blast"]
#'
...
#' )
#' names(named.outs.dirs) <- c("SAMN20336351", "SAMN20336352")
#' seurat.object <- read_singleMB(named.outs.dirs)
#'
read_singleMB <- function(named.outs.dirs,
                          # read_Microbe
                          align.method = "kraken2",
                          max.dim = 30000) {
  seurat.object <- read_Host(named.outs.dirs)
