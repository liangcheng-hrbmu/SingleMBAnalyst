###########################################################
#' Plot alpha diversity of single cells
#'
...
#' @param feature Default: \code{NULL}. Microbe feature from different taxonomy levels
...
#' # plotFeatures(seurat.object, feature = "Escherichia_coli")
#' # plotFeatures(seurat.object, feature = "Bacillota", tax_level = "phylum")
#'
  ...
  if (!is.null(feature)) {
    ...
    if (tax_level == "species") {
      seurat.object@meta.data$microbe_feature <- microbe.phyloseq.object %>%
        phyloseq::get_sample(feature)
    } else {
      seurat.object@meta.data$microbe_feature <- phyloseq_select_tax(microbe.phyloseq.object, feature, tax_level)
    }
    p <- seurat.object %>%
      Seurat::FeaturePlot(features = c("microbe_feature"), reduction = reduction, ...) +
      theme(aspect.ratio = 1) + labs(title = paste0(feature, " - ", tax_level))
  } else {
    p_list <- list()
...
plotTrajectory <- function(seurat.object,
                           ...
                           custom.color = NULL,
                           ...) {
  if (is.null(seurat.object@misc$host$trajectory$monocle)){
  ...
  if (is.null(gene_name) && is.null(tax_name)) {
    p_trajectory <- monocle3::plot_cells(cds, color_cells_by = feature,
  ...
  if (!is.null(tax_name)) {
    ...
    p_trajectory <- monocle3::plot_cells(cds, color_cells_by = "taxo_abu",
                                             label_cell_groups=FALSE,
                                             label_branch_points = FALSE
    ) +
      scale_color_gradientn(name = paste0(tax_level, " - ", tax_name),
