...
annoCell <- function(seurat.object,
  ...
  if (clustered && (!is.null(seurat.object$seurat_clusters))) {
    ...
    print(table(pred$labels) %>% as.data.frame() %>%
...
runLEfSe <- function(seurat.object,
  ...
  metaData <- data.frame(
    ...
    group.by = stringr::str_split(colnames(otu_tab.norm), "-", n = 2, simplify = T)[, 2],
...
runTrajectory <- function(seurat.object,
  ...
  cds <- monocle3::new_cell_data_set(expression_data = expression_data,
