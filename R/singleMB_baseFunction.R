...
constructPhyloseq <- function(microbe.counts) {
  ...
  phyloseq.object <- phyloseq::phyloseq(
    phyloseq::otu_table(microbe.counts, taxa_are_rows = T),
  ...
  phyloseq::sample_data(phyloseq.object) <- data.frame(
    ...
    orig.ident = (phyloseq.object@otu_table@.Data %>%
      colnames() %>%
      stringr::str_split("_", simplify = T))[, 1],
...
#' @export
#'
phyloseq_select_level <- function(phyloseq.object, tax_level = "genus") {
  if (tax_level == "species") {
    return(phyloseq.object)
  }
  # tax_levels <- c("kingdom", "phylum", "class", "order", "family", "genus", "species")
  # tax.tab <- phyloseq::tax_table(phyloseq.object) %>% as.data.frame()
  # otu.tab <- phyloseq::otu_table(phyloseq.object) %>% as.data.frame()
  #
  ...
  #
  # phyloseq.object.tax_level <- phyloseq::phyloseq(
  #   phyloseq::otu_table(otu.tab, taxa_are_rows = T),
  #   phyloseq::tax_table(tax.tab %>% as.matrix()),
  #   phyloseq::sample_data(phyloseq.object@sam_data)
  # )
  phyloseq.object <- data@misc$microbe$phyloseq.object
  phyloseq.object.tax_level <- phyloseq.object %>% phyloseq::tax_glom(taxrank = tax_level)
  rownames(phyloseq.object.tax_level@otu_table@.Data) <- phyloseq.object.tax_level@tax_table@.Data %>%
    as.data.frame() %>% .[, tax_level]
  rownames(phyloseq.object.tax_level@tax_table@.Data) <- phyloseq.object.tax_level@tax_table@.Data %>%
    as.data.frame() %>% .[, tax_level]

  return(phyloseq.object.tax_level)
}
